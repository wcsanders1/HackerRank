...
let getProfessorClasses cases =
    let rec f case classes = 
        if case = 0 
        then classes 
        else f (case - 1) (getProfessorClass() :: classes)
    f cases []
...
let classCanceled professorClass = 
    ...
    if professorClass.students - lateStudents < professorClass.threshold 
    then printf "YES\n" 
    else printf "NO\n"
...
let main _ =
    ...
    let classes = 
        getProfessorClasses cases 
        |> List.rev
    classes 
        |> List.map (fun c -> classCanceled c) 
        |> ignore
    0